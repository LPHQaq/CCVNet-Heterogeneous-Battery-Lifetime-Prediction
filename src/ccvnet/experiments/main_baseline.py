...
from ccvnet.experiments.paper_training import run_main_baseline
from ccvnet.model_registry import model_spec_to_dict, paper_experiment_model_specs


EXPERIMENT_NAME = "main_baseline"
...
def run_major(config: dict):
    return run_main_baseline(config, result_set="major")


def run_ablation(config: dict):
    return run_main_baseline(config, result_set="ablation")
