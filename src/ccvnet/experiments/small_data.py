...
from ccvnet.experiments.paper_training import run_small_data
...
def run_major(config: dict):
    return run_small_data(config, result_set="major")


def run_ablation(config: dict):
    return run_small_data(config, result_set="ablation")
