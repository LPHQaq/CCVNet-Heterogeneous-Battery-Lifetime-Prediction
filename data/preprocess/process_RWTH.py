...
DATASET_SETTINGS = {
    "dataset_name": "RWTH(NMC)",
    "cvd_curve_cycles": list(range(10, 101, 10)),
    "feature_cycles": [20, 50, 100],
    "reference_cycle": 10,
    "voltage_window": (3.5, 3.9),
    "n_voltage_grid": 200,
    "life_threshold": 80.0,
    "soh_baseline_note": "SOH baseline = fixed nominal-capacity normalization using 1.1 Ah for all RWTH cells",
    "segmentation_note": "segmentation rule = keep the longest current-gated discharge run in 3.5-3.9 V, prepend the last available Vmax anchor, then align on the overlapping voltage window",
    "plot_group_note": "plot grouping = RWTH is treated as a single total population without subgroup styles",
    "plot_groups": [],
    "processing_config": {
        "smooth_polyorder": 3,
...
def load_rwth_pickle_cycles_by_name(name):
...
def _find_vmax_anchor_index(voltage, capacity, run_start):
    finite_prefix = np.isfinite(voltage) & np.isfinite(capacity) & (np.arange(len(voltage)) <= run_start)
    finite_prefix &= (voltage >= Vmin) & (voltage <= Vmax)
    if np.sum(finite_prefix) == 0:
        return None

    actual_vmax = float(np.nanmax(voltage[finite_prefix]))
    candidate_mask = finite_prefix & (np.abs(voltage - actual_vmax) <= PROCESSING_CONFIG["vmax_check_tol"])
...
def _clean_common_discharge_branch(cycle_data):
    ...
    if np.any(np.isfinite(current)):
        ...
        finite_mask &= current < processing["current_threshold"]
    finite_mask &= (voltage >= Vmin) & (voltage <= Vmax)
    if np.sum(finite_mask) < min_branch_points:
        return None

    run = _find_best_true_run(finite_mask, voltage)
    if run is None:
        return None
    start, end = run

    anchor = _find_vmax_anchor_index(voltage, capacity, start)
    if anchor is None:
        return None
    ...
    branch = {
        ...
        "vmax": actual_vmax,
        "mode": "longest_current_gated_branch_plus_vmax_anchor",
    }
