...
CVDCYCLE_KEEP_KEYS = ("current_in_A", "voltage_in_V", "discharge_capacity_in_Ah", "charge_capacity_in_Ah", "time_in_s")


STAN_PLOT_GROUPS = ["Regular", "Regular_Ref"]


DATASET_SETTINGS = {
    "dataset_name": "STAN(NMC)",
    "cvd_curve_cycles": list(range(10, 101, 10)),
    "feature_cycles": [20, 50, 100],
    "reference_cycle": 10,
    "voltage_window": (3.0, 4.4),
    "n_voltage_grid": 200,
    "life_threshold": 80.0,
    "soh_baseline_note": "SOH baseline = fixed nominal-capacity normalization at 0.24 Ah from the STAN metadata.",
    "segmentation_note": "segmentation rule = no dataset-level reindexing is needed; first locate the stable negative-current discharge block, then anchor Q=0 at the preceding Vmax platform and keep the monotone discharge branch.",
    "plot_group_note": "plot grouping = use Regular and Regular_Ref only for summaries; the main pipeline keeps the template Total view.",
    "plot_groups": STAN_PLOT_GROUPS.copy(),
    "processing_config": {
        "smooth_polyorder": 3,
        ...
        "difference_zero_tol": 1e-8,
        "export_curve_dtype": "float64",
    },
}
...
def dataset_major_group(cell_name):
    return "Stanford_Nova"


def dataset_plot_group(cell_name):
    stem = Path(cell_name).stem
    if "Regular_Ref" in stem or "_Ref_" in stem or stem.endswith("_Ref"):
        return "Regular_Ref"
    if "Regular" in stem:
        return "Regular"
    return None
...
def compute_difference_profile(cell_cycles, target_cycle):
    ...
    difference = np.asarray(tgt_interp - ref_interp, dtype=np.float64)
    difference_zero_tol = float(PROCESSING_CONFIG.get("difference_zero_tol", 0.0) or 0.0)
    if difference_zero_tol > 0.0:
        difference[np.abs(difference) <= difference_zero_tol] = 0.0
    negative_difference = np.where(np.isnan(difference), difference, np.where(difference < -difference_zero_tol, difference, 0.0)).astype(np.float64)
...
def load_analysis_cycles(name):
    cell_cycles = load_pickle_cycles_by_name(name)
    return {"name": name, "cycles": cell_cycles}


def unpack_analysis_cycles(cell_payload):
    if isinstance(cell_payload, dict):
        return cell_payload.get("name"), cell_payload.get("cycles")
    return None, cell_payload


def release_analysis_cycles(cell_payload):
    if isinstance(cell_payload, dict):
        name = cell_payload.get("name")
        cell_cycles = cell_payload.get("cycles")
    else:
        name = None
        cell_cycles = cell_payload
    if cell_cycles is not None:
        CELL_CAPACITY_SUMMARY_CACHE.pop(("trace", id(cell_cycles)), None)
        CELL_CAPACITY_SUMMARY_CACHE.pop(("soh", id(cell_cycles)), None)
        cell_name_by_obj_id.pop(id(cell_cycles), None)
    if name is not None:
        CELL_RAW_CACHE.pop(name, None)


def _find_best_true_run(mask, voltage):
    ...
    best_run = max(
        runs,
        key=lambda run: (
            float(np.nanmax(voltage[run]) - np.nanmin(voltage[run])),
            int(len(run)),
        ),
    )
    return int(best_run[0]), int(best_run[-1] + 1)


def _find_preceding_vmax_anchor(voltage, capacity, discharge_start):
    q_zero_tol = float(PROCESSING_CONFIG["q_zero_tol"])
    vmax_check_tol = float(PROCESSING_CONFIG["vmax_check_tol"])

    valid_prefix = np.isfinite(voltage) & np.isfinite(capacity) & (np.arange(len(voltage)) <= discharge_start)
    valid_prefix &= (voltage >= float(Vmin)) & (voltage <= float(Vmax) + vmax_check_tol)
    if np.sum(valid_prefix) == 0:
        return None

    actual_vmax = float(np.nanmax(voltage[valid_prefix]))
    candidate_mask = (
        valid_prefix
        & (np.abs(capacity) <= q_zero_tol)
        & (np.abs(voltage - actual_vmax) <= vmax_check_tol)
    )
    candidate_idx = np.flatnonzero(candidate_mask)
    if candidate_idx.size:
        return int(candidate_idx[-1]), actual_vmax

    prefix_idx = np.flatnonzero(valid_prefix)
    if prefix_idx.size == 0:
        return None
    vmax_idx = prefix_idx[np.argmax(voltage[prefix_idx])]
    return int(vmax_idx), actual_vmax
...
def _clean_common_discharge_branch(cycle_data):
    ...
    current_threshold = float(processing["current_threshold"])
    q_zero_tol = float(processing["q_zero_tol"])
    min_branch_points = 5

    discharge_mask = finite_mask & (voltage >= float(Vmin)) & (voltage <= float(Vmax))
    if np.any(np.isfinite(current)):
        discharge_mask &= np.isfinite(current)
        discharge_mask &= current < current_threshold
    if np.sum(discharge_mask) < min_branch_points:
        return None

    run = _find_best_true_run(discharge_mask, voltage)
    if run is None:
        return None
    discharge_start, discharge_end = run

    anchor = _find_preceding_vmax_anchor(voltage, capacity, discharge_start)
    if anchor is None:
        return None
    ...
    last_q = float(capacity[anchor_idx])

    for idx in range(discharge_start, discharge_end):
        v = float(voltage[idx])
        q = float(capacity[idx])
        ...
        if (v <= last_v + voltage_tol) and (q >= last_q - capacity_tol):
            if not (abs(v - last_v) <= voltage_tol and abs(q - last_q) <= capacity_tol):
                keep_idx.append(idx)
                last_v = v
                last_q = q

    if len(keep_idx) < min_branch_points:
        return None

    keep_idx = np.asarray(keep_idx, dtype=int)
    plot_axis = voltage[keep_idx].copy()
    plot_value = capacity[keep_idx].copy() - float(capacity[anchor_idx])
    if plot_value.size < 2:
        return None
    if not np.any(plot_value[1:] > q_zero_tol):
        return None

    grouped = (
        pd.DataFrame({
            "V": np.round(plot_axis, processing["voltage_round_decimals"]),
            "V_raw": plot_axis,
            "Q": plot_value,
        })
    ...
    if len(grouped) < min_branch_points:
        return None
    ...
    branch = {
        ...
        "slice": (int(keep_idx[0]), int(keep_idx[-1] + 1)),
        "raw_points": int(len(keep_idx)),
        "start_index": int(anchor_idx),
        "keep_indices": keep_idx,
        "vmax": actual_vmax,
        "mode": "current_transition_to_negative_block_then_monotone_discharge",
    }
...
def extract_segment_pair(ref_cycle_data, target_cycle_data):
    ...
    overlap_pad = float(PROCESSING_CONFIG["overlap_pad"])
    v_lo = max(float(np.nanmin(ref_axis_interp)), float(np.nanmin(tgt_axis_interp))) + overlap_pad
    v_hi = min(float(np.nanmax(ref_axis_interp)), float(np.nanmax(tgt_axis_interp))) - overlap_pad
