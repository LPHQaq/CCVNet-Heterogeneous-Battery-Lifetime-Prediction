...
DATASET_SETTINGS = {
    ...
    "n_voltage_grid": 200,
    "life_threshold": 80.0,
    "soh_baseline_note": "SOH baseline = nominal-capacity normalization with 3.5 Ah for Tongji1/Tongji2 and 2.5 Ah for Tongji3",
    "segmentation_note": "segmentation rule = Tongji extraction starts from the last Q=0 point on the Vmax platform and then keeps the monotone discharge branch",
    "plot_group_note": "plot grouping = Tongji subgroup styles are defined in the single settings cell",
    "plot_groups": TONGJI_SUBGROUPS.copy(),
    "save_excluded_groups": ["Tongji1_CY25-1"],
    "processing_config": {
        "smooth_polyorder": 3,
...
def dataset_major_group(cell_name):
    ...
    if stem.startswith("Tongji1_"):
        return "Tongji1"
    if stem.startswith("Tongji2_"):
        return "Tongji2"
...
def dataset_plot_group(cell_name):
    stem = Path(cell_name).stem
    return next((group_name for group_name in TONGJI_SUBGROUPS if group_name in stem), None)
...
def compute_difference_profile(cell_cycles, target_cycle):
    if "extract_segment_pair" not in globals():
        raise RuntimeError("Run the Tongji-specific extraction cell before calling compute_difference_profile.")
    ...
    smooth_polyorder = PROCESSING_CONFIG["smooth_polyorder"]
    ref_interp = advanced_smooth_capacity_curve(ref_interp_raw, smooth_method="savgol", smooth_polyorder=smooth_polyorder, smooth_window_ratio=q_interp_window_ratio, spike_abs=PROCESSING_CONFIG["spike_abs"], spike_k=PROCESSING_CONFIG["spike_k"])
    tgt_interp = advanced_smooth_capacity_curve(tgt_interp_raw, smooth_method="savgol", smooth_polyorder=smooth_polyorder, smooth_window_ratio=q_interp_window_ratio, spike_abs=PROCESSING_CONFIG["spike_abs"], spike_k=PROCESSING_CONFIG["spike_k"])
...
def _find_cvd_anchor_index(voltage, capacity):
    q_zero_tol = float(globals().get("q_zero_tol", 1e-5))
    vmax_check_tol = float(globals().get("vmax_check_tol", 5e-3))

    finite_mask = np.isfinite(voltage) & np.isfinite(capacity)
    if np.sum(finite_mask) == 0:
        return None

    actual_vmax = float(np.nanmax(voltage[finite_mask]))
    candidate_mask = (
        finite_mask
    ...
    start = int(candidate_idx[-1])
    later_vmax_mask = (
        finite_mask
        & (np.arange(len(voltage)) > start)
        & (np.abs(voltage - actual_vmax) <= vmax_check_tol)
    )
    if np.any(later_vmax_mask):
        return None

    return start, actual_vmax
...
def _clean_common_discharge_branch(cycle_data):
    ...
    anchor = _find_cvd_anchor_index(voltage, capacity)
    if anchor is None:
        return None
    start, actual_vmax = anchor

    finite_mask = np.isfinite(voltage) & np.isfinite(capacity)
    if np.sum(finite_mask) == 0:
        return None

    vmin_clip = float(globals().get("Vmin_clip", float(np.nanmin(voltage[finite_mask]))))
    vmax_clip = float(globals().get("Vmax_clip", float(np.nanmax(voltage[finite_mask]))))
    vmax_check_tol = float(globals().get("vmax_check_tol", 5e-3))
    voltage_tol = float(globals().get("voltage_monotonic_tol", 5e-3))
    capacity_tol = float(globals().get("capacity_monotonic_tol", 1e-6))
    current_threshold = float(globals().get("current_threshold", -0.05))
    upper_clip = max(vmax_clip, actual_vmax + vmax_check_tol)
    has_current = bool(np.any(np.isfinite(current)))

    keep_idx = [start]
    last_v = float(voltage[start])
    last_q = float(capacity[start])

    for idx in range(start + 1, len(voltage)):
        v = float(voltage[idx])
        ...
        if has_current:
            current_i = float(current[idx])
            if not np.isfinite(current_i):
                if len(keep_idx) > 1:
                    break
                continue
            if current_i >= current_threshold:
                if len(keep_idx) > 1:
                    break
                continue
    ...
    plot_value = capacity[keep_idx].copy() - float(capacity[start])
    if plot_value.size < 2 or np.any(plot_value[1:] <= float(globals().get("q_zero_tol", 1e-5))):
        return None

    round_decimals = int(globals().get("voltage_round_decimals", 4))
    grouped = (
        pd.DataFrame({
            "V": np.round(plot_axis, round_decimals),
            "V_raw": plot_axis,
            "Q": plot_value,
        })
    ...
    branch = {
        ...
        "vmax": actual_vmax,
        "mode": "last_q0_on_vmax_platform_then_monotone_discharge",
    }
...
def extract_segment_pair(ref_cycle_data, target_cycle_data):
    ...
    overlap_pad = float(globals().get("overlap_pad", 0.0))
